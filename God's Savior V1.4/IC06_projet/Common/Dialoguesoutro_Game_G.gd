...
func next_line():
	current_dialogue_id +=1
	if current_dialogue_id >= len(dialogues["outro_G"][0]["Game"+str(N_dlg)]):
		$Timer_G.start()
		$NinePatchRect.visible=false
		return
	$NinePatchRect/Name.text= ""
	$NinePatchRect/Message.text=dialogues["outro_G"][0]["Game"+str(N_dlg)][current_dialogue_id]
	$NinePatchRect/Message.percent_visible = 0
	txt_length = str(dialogues["outro_G"][0]["Game"+str(N_dlg)][current_dialogue_id]).length()
